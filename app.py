...
DATABLIX_BUILD = "Visible Source Change Matrix 2026.08.07-v76"
...
def _render_company_source_field_comparison(
    ...
    if not material.empty:
        ...
        st.caption(
            "Every meaningful difference is listed below by property. Use the notes box to record "
            "what you verified, why a value changed, or what still needs follow-up."
        )
        ...
        for property_name, group in material.groupby("Starting Record", sort=False):
            ...
            with st.expander(
                f"{property_name} · {property_change_count} change(s)",
                expanded=True,
            ):
                st.dataframe(
                    detail,
                ...
                note_identity = research_record_id or (
                    f"{property_name}|{int(research_position) if pd.notna(research_position) else 'x'}"
                )
